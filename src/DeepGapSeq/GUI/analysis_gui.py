...
from qtpy.QtWidgets import (QWidget, QVBoxLayout, QSizePolicy,QSlider, QLabel, QFileDialog)
import numpy as np
import traceback
...
class AnalysisGUI(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.actionGapSeq.triggered.connect(self.load_simulated_data)
    ...
    def filter_data_dict(self):
        ...
        if user_filter != "None" or nucleotide_filter != "None":
            ...
            self.n_traces = len(self.data_dict[plot_data]) - 1
    ...
    def initialise_plot(self):
        ...
        try:
            ...
            n_traces = len(self.data_dict[plot_data]) - 1

            plot_labels = list(self.data_dict[plot_data][0].keys())
    ...
    def plot_traces(self, update_plot = False):
        ...
        try:
            ...
            if hasattr(self, "plot_grid") == False or update_plot == True:
                self.plot_grid = self.update_plot_layout(self.data_dict)
            ...
            if self.plot_grid != {}:
                ...
                for plot_index, grid in enumerate(self.plot_grid.values()):
                    ...
                    user_label = self.data_dict[plot_name][localisation_number]["user_label"]
                    nucleotide_label = self.data_dict[plot_name][localisation_number]["nucleotide_label"]

                    plot_details = f"{plot_name} [#:{localisation_number} C:{user_label}  N:{nucleotide_label}]"
                    ...
                    for axes_index, (plot, line,  plot_label) in enumerate(zip(sub_axes, plot_lines, plot_lines_labels)):
                        ...
                        legend = plot.legend
                        data = self.data_dict[plot_name][localisation_number][plot_label]

                        break_points = self.data_dict[plot_name][localisation_number]["break_points"]
                        state_means = self.data_dict[plot_name][localisation_number]["state_means"][plot_label]
                        gamma_correction_ranges = self.data_dict[plot_name][localisation_number]["gamma_correction_ranges"]
    ...
    def update_plot_layout(self, plot_data):
        ...
        try:
            ...
            for plot_index, plot_name in enumerate(plot_data.keys()):
                ...
                user_label = plot_data[plot_name][localisation_number]["user_label"]
                nucleotide_label = plot_data[plot_name][localisation_number]["nucleotide_label"]
                ...
                for axes_index, plot in enumerate(sub_plots):
                    ...
                    plotmeta[axes_index] = {"plot_name": plot_name, "line_label": line_label}
